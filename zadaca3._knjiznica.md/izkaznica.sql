...
FROM clanovi c
JOIN posudbe p ON c.id = p.clan_id
JOIN kopija kp ON p.kopija_id = kp.id
JOIN knjige k ON kp.knjiga_id = k.id;


SELECT c.ime, c.prezime, k.naslov, p.datum_povrata
FROM clanovi c
JOIN posudbe p ON c.id = p.clan_id
JOIN kopija kp ON p.kopija_id = kp.id
JOIN knjige k ON kp.knjiga_id = k.id
WHERE p.datum_povrata < CURDATE() AND p.datum_povrata IS NOT NULL;


SELECT z.naziv AS zanr, COUNT(*) AS broj_dostupnih_knjiga
FROM zanrovi z
JOIN knjige k ON z.id = k.zanr_id
JOIN kopija kp ON k.id = kp.knjiga_id
